...
if (isset($_POST['itemtype']) && isset($_POST['tab']) && isset($_POST['color'])
   ...
   $tabs          = $tabs[$_POST['tab']];
   $types         = PluginIframeTab::getTypes();
   $itemtype      = $types[$_POST['itemtype']];
   $_POST['name'] = $itemtype
      . "-" . $tabs
      . "-" . ucfirst(__($_POST['color'], 'iframe'));
}

$tabs = new PluginIframeTab;

if (isset($_POST["add"])) {
   $newID = $tabs->add($_POST);
   Html::redirect($CFG_GLPI["root_doc"]."/plugins/iframe/front/tab.form.php");

} elseif (isset($_POST["delete"])) {
   $ok = $tabs->delete($_POST);
   Html::redirect($CFG_GLPI["root_doc"]."/plugins/iframe/front/tab.php");
